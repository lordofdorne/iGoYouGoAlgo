...
const paintHouses = costs => {
  ...
  let greenCost = costs[0][2]

  for (let i = 1; i < costs.length; i++) {
    let paintRed = Math.min(blueCost, greenCost) + costs[i][0]
    let paintBlue = Math.min(redCost, greenCost) + costs[i][1]
    let paintGreen = Math.min(redCost, blueCost) + costs[i][2]

    redCost = paintRed
    blueCost = paintBlue
    greenCost = paintGreen
  }
